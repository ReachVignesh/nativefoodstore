...
class ControllerCheckoutPaymentMethod extends Controller {
	public function index() {
		...
		if (isset($this->session->data['payment_address'])) {
			...
			//pincode checking for COD
			$pincode_list = $this->model_extension_extension->getPincode();
			$pincode = array_column($pincode_list, 'pincode');
			
			$logged = $this->customer->isLogged();
			if($logged) { 
				$this->load->model('account/address');

				$addresses_list = $this->model_account_address->getAddresses();
				...
				if ($this->config->get($result['code'] . '_status')) {
					...
					if ($method) {
						if ($recurring) {
							...
							// if($zone_id == 1503) {
							// 	$method_data[$result['code']] = $method;
							// } else if(in_array($postcode, $pincode)) {
								$method_data[$result['code']] = $method;
							// }
						}
					}
		...
		if (empty($this->session->data['payment_methods'])) {
			$data['error_warning'] = sprintf($this->language->get('error_payment_delivery'), $this->url->link('information/contact'));
		} else {
			$data['error_warning'] = '';
		}
