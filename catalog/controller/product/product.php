	...
	public function index() {
		...
		if ($product_info) {
         ...
         if($manufacturer_image){
            $data['manufacturers_img'] = $this->model_tool_image->resize($manufacturer_image['image'], 110, 60);
         }else{
			...
			if ((float)$product_info['special']) {
				...
				$data['discount'] = round(($discount_price/$product_info['price']) * 100);
			} else {
				$data['special'] = false;
				$data['discount']=false;
			}
		...
		$manufacturer_image = $this->model_catalog_manufacturer->getManufacturer($product_info['manufacturer_id']);
         ...
         if($manufacturer_image){
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					if ($option_value['subtract'] || ($option_value['quantity'] > 0)) {
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()  && $this->customer->isVerified()==1) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
							$price = $this->currency->format($this->tax->calculate($option_value['price'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false));
						} else {
							$price = false;
						}
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/product/product_quick_view.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/product/product_quick_view.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/product/product_quick_view.tpl', $data));
			}
		} else {
			$url = '';
	...
	public function checkzipcode()
		{
			...
			$this->load->model('catalog/product');
			$zips=$this->model_catalog_product->getCheckingPincode($zipcode);
			...
			if($zips){
				if (!$json) {
					...
					$json['error'] = "Delivery Not Available In Your Area";
				}
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_encode($json));
		}
