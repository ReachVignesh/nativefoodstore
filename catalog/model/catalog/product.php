...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['amtmin']) && !empty($data['amtmax'])) {
			$sql .= " AND (p.price >= '" .$data['amtmin'] . "' AND p.price <= '" .$data['amtmax'] . "')";
		}
//echo $sql;die;
...
//print_r($product_option_value_data);die;
	...
	public function getZone($zone_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "state_city WHERE pincode = '" . (int)$zone_id . "'");
		
		return $query->row;
	}

	public function getCheckingPincode($pincode) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "delivery_pincode WHERE pincode = '". $pincode ."'");

		return $query->row;	
	}

	public function getHotProducts() {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "module WHERE code = 'hot_product'");
		
		$result = unserialize($query->row['setting']);
		$product_data = array();

		if(!empty($result['product'])) {
			foreach ($result['product'] as $product_id) {
