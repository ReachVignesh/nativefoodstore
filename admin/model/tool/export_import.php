...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$price = trim($product['price']);
		$special_price = trim($product['special_price']);
		...
		$sql = "UPDATE `".DB_PREFIX."product` SET `quantity`=$quantity,`price`=$price,`date_modified`=NOW() WHERE `product_id`=$product_id";
		$sql1 = "UPDATE `".DB_PREFIX."product_special` SET `price`=$special_price WHERE `product_id`=$product_id";
		$sql2 = "UPDATE `".DB_PREFIX."product_description` SET `name`='$name' WHERE `product_id`=$product_id";
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$product_code = $this->getCell($data,$i,$j++,'');
			...
			$special_price = $this->getCell($data,$i,$j++,'0.00');
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array
		( "product_id", "name", "categories", "product_code", "upc" );
		if (in_array("ean",$product_fields)) {
...
//		}
	...
	function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$product_fields = array('product_id', 'name', 'product_code', 'quantity');
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('product_code'),10)+1);
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_export_import_3_5&language_code=$language_code");
