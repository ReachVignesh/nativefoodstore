...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$data['text_missing_status'] = $this->language->get('text_missing_status');
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) && ($this->request->get['token'] != $this->session->data['token'])) {
			...
			/*
			Author : Mohan
			Description : Customized missing order in header notification 
						  Added below missing orders information
			Missing Orders */
			$data['missing_status_total'] = $this->model_sale_order->getMissingOrderTotals();
			$data['missing_status'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&filter_order_status=0', 'SSL');
			...
			//echo $data['complete_status_total'];die;
			// Returns
